...
class RandPhpText
{
    ...
    public static function getRandomLine()
    {
        self::ensureTextsLoaded();

        $probabilityArray = [];

        foreach (self::$texts as $key => $tweet) {
...
//        var_dump(self::$texts[$tweetKey]);
        return self::$texts[$tweetKey]['text'];
    }

    private static function ensureTextsLoaded()
    {
        if (null === self::$texts) {
            self::$texts = include __DIR__ . '/tweets.php';
        }
    }
}
...
return function ($event) use ($connection)
{
    $result = [];
    $randomLine = RandPhpText::getRandomLine();
    $result[] = $randomLine;
